...
while [[ $run_ganak == "true" ]]; do
    ...
    echo "c Command: ../bin/doalarm $tout_ganak ../bin/ganak -m $hash_range -maxdec 5000000 500 --seed $seed $input_file > output"
    `./bin/doalarm $tout_ganak ./bin/ganak -m $hash_range -cs $maximum_component_cache_size -maxdec 5000000 500 --seed $seed $input_file > output` > /dev/null 2>&1
...
if [[ $useIS == *"Terminating solver because the number of decisions"* ]]; then
    ...
    `./bin/doalarm $tout_arjun ./bin/arjun --seed $seed $input_file > independent_support` > /dev/null 2>&1
    touch newfile
...
while [[ $run_ganak == "true" ]]; do
    ...
    `./bin/doalarm $tout_ganak ./bin/ganak -cs $maximum_component_cache_size -m $hash_range --seed $seed $input_file > output` > /dev/null 2>&1
    # cat output
    hash_error=`grep "ERROR: We need to change the hash range" output`
    if [[ $hash_error == *"ERROR: We need to change the hash range"* ]]; then
        let hash_range=2*hash_range
